...
\label{sec:johdanto}
  ...
  Kuva~\ref{fig:yleiskuva_termien_suhteista} esittelee tutkielmaan liittyvien autentikoinnin toimijoiden välisiä suhteita. Ylimpänä on identiteettiään hallinnoiva loppukäyttäjä, eli ihminen, joka ottaa käyttäjäagentillaan yhteyden esimerkiksi GMail-palveluun. GMail-palvelu on identiteetinkuluttaja (\emph{identity consumer}), joka pitää hallussaan käyttäjän yksilöivää tunnistetta (\emph{identifier}) -- vaan ei varmennetta (\emph{credentials}). Varmenne voi olla esimerkiksi salasana, joka vahvistetaan yhteentoimivuuden standardin avulla identiteetintarjoajan (\emph{identity provider}) palvelussa. Varmenteen vahvistaminen on identiteetintarjoajan sisäinen toteutus, joka ei kuulu tämän tutkielman piiriin. Yleisiä toteutuksia siihen ovat esimerkiksi Kerberos-tiketit ja salasanan tiivisteen tallettaminen tietokantaan tai LDAP-palveluun.
...
Tässä esitellään tutkimuksen tausta.

% pura mooc nimi tässä uudelleen auki
todo kömpelö kappale:
MOOC:n lähtökohtana on nimensä mukaisesti olla valtava (\emph{massive}). Jos virtuaalikurssi muodostuu suosituksi täytyy kuormitukseen liittyvät pullonkaulat tunnistaa. Kun toimintojen vastuualueet on SOA-mallin mukaan jaettu omiksi palveluikseen, skaalaminen on monoliittista järjestelmää suoraviivaisempaa. Valtavuuden vuoksi on houkuttelevaa ulkoistaa identiteetintarjoajan vastuualue täysin kolmannelle osapuolelle kuten Helsingin yliopistolle, Googlelle tai Facebookille. Helsingin yliopistolla on noin 40.000 käyttäjätunnusta\footnote{Helsingin yliopistolla 35.000 opiskelijaa ja 4.000 henkilökunnan jäsentä (\url!http://www.helsinki.fi/lyhyesti/!)}. Facebookilla ja Googlella yhteensä yli 1,2 miljardia käyttäjätunnusta\footnote{Facebookilla 845 miljoonaa käyttäjää (\url!http://newsroom.fb.com/content/default.aspx?NewsAreaId=22!), Googlen Gmail-palvelulla 350 miljoonaa käyttäjää (\url!http://thenextweb.com/google/2012/01/19/gmail-closes-in-on-hotmail-with-350-mm-active-users/!)}. Pelkästään kahden jälkimmäisen kautta iso prosenttiosuus MOOC-palvelun käyttäjistä saataisiin autentikoitua -- erityisesti koska kurssi on avoin kaikille.

Tavoite: ettei autentikointi pullonkaulaksi kun opiskelijamäärä kasvaa.


\subsection{Kertakirjautuminen MOOC-järjestelmään} % (fold)
\label{sub:tutkimusongelmana_kertakirjautuminen}

Tässä esitellään tutkimusongelma ja mainitaan termejä, jotka selitetään seuraavissa luvuissa.

Tässä kerrotaan mihin tutkielmassa otetaan kantaa, mitä siinä pohditaan ja että lopputuloksena on suositus siitä miten MOOC-sisäänkirjautuminen kannattaa toteuttaa.

Tämän luvun lukemisen jälkeen lukija tietää, mikä on tutkimuksen päätehtävä.
...
\label{sub:web_ja_saas}
  ...
  % tässä selitetään mikä on identiteetinkuluttaja,
  %    ja että oauthissa asiakas ja openidssä relying party
  ...
  % soa & security: http://www.slideshare.net/oliverpfaff/soa-security-so-what-10143526
  % white paper \cite{federated_auth_case_2008}
  ...
  %
  % todo upota tämä lukuun rajapinnat ja web
  %
  ...
  % decouling between means of production and use of product.
  % ei tartte omistaa lehmää jotta voi juoda maitoa.
  % vs ei tartte identiteettidataa jotta voi autentikoida
