...
\label{sec:autentikoituminen_ja_web}
  ...
  HTTP-yhteys web-palvelimen ja käyttäjäagentin välillä voidaan suojata Secure Socekts Layer (SSL) tai Transport Layer Security (TLS) -protokollalla, kun web-palveluntarjoaja autentikoidaan sertifikaatin avulla. Tällöin web-palvelin ja käyttäjäagentti neuvottelevat keskenään sertifikaatin sisältämän epäsymmetrisen avaimen perusteella jaetun salaisuuden. Jaettuna salaisuutena toimii symmetrinen avain, jonka perusteella kaikki HTTP-protokollan siirtämät tiedot salakirjoitetaan. Tällaisesta suojatusta kommunikaatiokanavasta web-palvelimen ja käyttäjäagentin välillä käytetään nimitystä \emph{HTTPS-yhteys}~\cite{rfc_https_2818}.
...
\label{sec:mooc_palvelun_autentikointiratkaisun_arkkitehtuuri}
  ...
  \paragraph{Helsingin yliopiston identiteettipalvelu} % (fold)
  ...
  Selitä SAML-kuva % http://code.google.com/googleapps/domain/sso/saml_reference_implementation.html
