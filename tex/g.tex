...
\subsubsection{Keksien käyttö autentikoinnissa} % (fold)
...
% subsubsection keksien_käyttö_autentikoinnissa (end)
...
\label{sec:mooc_palvelun_autentikointiratkaisun_arkkitehtuuri}
  ...
  \subsection{Facebook-identiteetin autentikoiminen} % (fold)
  \label{ssub:mooc_fb_auth}

  Facebook Platform soveltaa OAuth 2.0 -protokollaa autentikointiin ja auktorisointiin~\cite{facebook_oauth2_doc}. Tietojenkäsittelytieteen laitoksen MOOC-järjestelmä on web-palvelu, joten OAuth-virtaukseksi soveltuu parhaiten auktorisointikoodin käyttäminen. Kaikki kommunikointi Facebook Platform -rajapinnan ja MOOC-palvelun välillä suojataan käyttämällä \verb!https!-yhteyttä.

  Ensimmäiseksi identiteetinkuluttajana ja resurssin asiakkaana toimiva MOOC-palvelu rekisteröidään Facebook Platform -identiteetintarjoajan asiakkaaksi~\cite{facebook_oauth2_doc}. Rekisteröintiprosessissa resurssin asiakkaalle määritetään yksilöivänä tunnisteena \verb!client_id!, jaettuna salaisuutena \verb!client_secret! sekä MOOC-palvelun palautusosoite \verb!redirect_uri!, johon identiteetintarjoaja uudelleenohjaa loppukäyttäjän autentikointipyynnön jälkeen.

  MOOC-palvelun tavoitteena on saada Facebook Platform -palvelulta pääsyoikeuspoletti \verb!access_token!. Pääsyoikeuspoletin saaminen edellyttää hyväksyntää sekä loppukäyttäjältä että palveluntarjoajalta. Tarjoajana Facebook Platform täytyy ensin hyväksyä loppukäyttäjän autentikoituminen, ja sen jälkeen loppukäyttäjän on suostuttava tietojen luovuttamiseen MOOC-palveluun.

  \subsubsection{Autentikoinnin vaiheet Facebook Platform -palveluun} % (fold)
  \label{ssub:autentikoinnin_vaiheet_facebook_platform_palveluun}

  todo
  Autentikointiprosessi alkaa, kun loppukäyttäjä klikkaa Facebook-ikonia MOOC-palvelun sisäänkirjautumissivulla. MOOC ohjaa käyttäjään Facebook-päätepisteeseen, joka kysyy käyttäjältä suostumuksen tietojen välittämiseen MOOCiin. MOOC-palvelu ja Facebook Platform neuvottelevat keskenään pääsyoikeuden myötämisestä ja lopulta MOOC saa pääsyoikeuspoletin. Kuvassa~\ref{fig:oauth_auth_code_flow} on esitetty Facebook Platform -autentikoinnin vaiheet, kun MOOC-palvelu soveltaa auktorisointikoodin virtausta.

  \begin{singlespacing}
    \begin{verbatim}
      kuva {fig:oauth_auth_code_flow}
      todo piirrä puhtaaksi \cite{ietf_oauth2}:
+----------+
| resource |
|   owner  |
| (opiskelija)  |
...
parts as they pass through the user-agent.
  ...
  \begin{description}
  \item[(A)]
   MOOC ohjaa käyttäjäagentin Facebook Platformin auktorisointipäätepisteeseen ja liittää pyyntöön oman tunnisteensa (\verb!client_id!) sekä paluuosoitteen (\verb!redirect_uri!).
    % https://www.facebook.com/dialog/oauth?
    %      client_id=YOUR_APP_ID&redirect_uri=YOUR_URL

  \item[(B)]
  Facebook Platform ottaa käyttäjäagentin vastaan ja autentikoi käyttäjän.
  Jos käyttäjä on jo sisäänkirjautunut, Facebook validoi käyttäjän session voimassaolon  käyttäjäagenttiin Facebookin tallettaman keksin perusteella (todo keksi vai eväste)~\cite{facebook_oauth2_doc}. Jos sessio ei ole voimassa, Facebook kysyy käyttäjätunnusta ja salasanaa.
  OAuth-standardi ei määrittele, kuinka autentikointi täytyy suorittaa~\cite{ietf_oauth2}.

  \item[(C)]
  Autentikoitumisen jälkeen Facebook kysyy käyttäjän suostumuksen MOOC-palvelun pyytämien tietojen luovuttamiseksi eteenpäin (kuva~\ref{fig:facebook_auth_dialog}). Pyydettävien tietojen laajuus määritetään \verb!scope!-parametrilla. Oletusarvoisesti valtuutuspyyntö koskee Facebookissa vain käyttäjän perustietoja kuten nimi, sukupuoli, profiilikuva~\cite{facebook_oauth2_doc}. MOOC-palvelu tarvitsee perustietojen lisäksi sähköpostiosoitteen, joten MOOC asettaa \verb!scope!-parametrin arvoksi \emph{email}\footnote{
  ...
  \item[(D)]
    MOOC-palvelu ottaa suoran yhteyden Facebook Platformiin eli käyttäjäagentti ei ole välissä. MOOC välittää edellisessä vaiheessa saamansa auktorisointikoodin ja pyytää saada käyttöönsä pääsyoikeuspoletin (\verb!access_token!). Auktorisointikoodin lisäksi MOOC välittää pyynnössään yksilöivän tunnisteensa (\verb!client_id!) ja jaetun salaisuuden (\verb!client_secret!), joiden perusteella Facebook Platform autentikoi MOOC-palvelun eli varmistuu siitä, että pyyntö tulee oikealta toimijalta. Ainoastaan MOOC ja Facebook Platform tuntevat jaetun salaisuuden, ja vastaavasti auktorisointikoodi on ainoastaan käyttäjäagentin, MOOC-palvelun ja Facebook Platformin tiedossa.

  \item[(E)]
  Facebook Platform luo pääsyoikeuspoletin (\verb!access_token!) ja lähettää sen MOOC-palveluun. MOOC käyttää pääsyoikeuspolettia, kun se kommunikoi loppukäyttäjän valtuuksin Facebook Platformin kanssa. Tämän vaiheen jälkeen MOOC tuntee käyttäjän perustiedot ja sähköpostiosoitteen.

  Vaiheen jälkeen käyttäjä on lisäksi auktorisoinut MOOC-palvelun omaan Facebook-profiiliinsa ja auktorisointi on voimassa kunnes käyttäjä poistaa sen. Tämän ansiosta myöhemmin MOOC-palveluun saapuva käyttäjä voidaan autentikoida kysymättä lainkaan salasanaa, jos käyttäjän oma Facebook-sessio on voimassa. Tällöin virtauksesta suoritetaan ainoastaan vaiheet \verb!(A)! ja \verb!(E)!.

  % TODO TÄSSÄ

  Jos asiakas tarvitsee käyttäjästä riippumattoman pääsyn resurssiin, asiakkaan on pyydettävä käyttäjän suostumusvaiheessa Facebook Platformilta \verb!offline access!-käyttöoikeutta.

  \item[(XX)]
  Autentikointi: pääsyoikeuspoletin vastaanottamisen jälkeen MOOC ottaa yhteyden Facebook Platformin
  \end{description}

  % Auktorisointikoodi on lyhyen aikaa voimassa oleva väliaikainen tunniste, jota vastaan kuluttajapalvelu seuraavaksi lunastaa valtakirjan loppukäyttäjän resursseihin eli \verb!access_token!- ja (mahdollisesti) \verb!refresh_token!-poletit. % todo tarkista mitä kirjoitettiin refresh-tokenista

  % MOOC-palvelu tarvitsee auktorisointikoodia ainoastaan silloin, kun käyttäjältä kysytään suostumusta.
  Käyttäjän suostumusvaihe suoritetaan aina, kun resurssin asiakkaan toimivalla MOOC-palvelulla ei ole pääsyoikeuspolettia (\verb!access_token!) tai kun aiemmin myönnetty pääsyoikeuspoletti on vanhentunut. Pääsyoikeuspoletti on rajallisen ajan voimassa ja sen uusiminen edellyttää joko käyttäjän suostusvaihetta tai virkistyspolettia. Jos asiakas tarvitsee käyttäjästä riippumattoman pääsyn resurssiin, asiakkaan on pyydettävä käyttäjän suostumusvaiheessa Facebook Platformilta \verb!offline access!-käyttöoikeutta. Tällöin tarjoaja antaa auktorisointikoodin lunastamisen yhteydessä \verb!acces_token!-poletin lisäksi myös \verb!refresh_token!-poletin.facebook ei käytä refreshtokenia vaan jos scopessa on offline access, niin pääsyoikeuspoletin voimassaoloaikaa ei määritellä (ks pitääkö paikkansa vielä deprekoinnin jälkeen).
