...
\label{sec:mooc_palvelun_autentikointiratkaisun_arkkitehtuuri}
  ...
  \begin{figure}[h!]
    ...
    \caption{Mahdolliset MOOC-palvelun autentikointirajapinnat. Kuvassa Helsingin yliopistolla on MOOC-palvelun kaltaisia kolmannen osapuolen toteuttamia palveluja varten identiteetintarjoajapalvelu, joka abstraktoi käyttäjärekisterin on OAuth-rajapinnan taakse.}
  ...
  Autentikointiratkaisun ulkoiset rajapinnat on esitetty kuvassa~\ref{fig:mooc_hy_idp}, ja valinnat on taustoitettu edeltävissä alaluvuissa. XXX TODO toinen virke vielä XXX
  ...
  Uloskirjautuminen MOOC-palvelusta on syytä toteuttaa kaksivaiheisesti. Uloskirjautuminen MOOC-palvelusta tapahtuu poistamalla käyttäjäagenttiin talletettu eväste. Tämän jälkeen MOOC ohjaa käyttäjäagentin identiteetintarjoajan uloskirjautumisrajapintaan, jotta käyttäjän sessio päätetään myös identiteetintarjoajallaadsfasdfadsf XXX TODO TÄHÄN JÄIN XXX.

  Session päättäminen yksistään MOOC-palvelussa ei riitä. Jos käyttäjän sessio on uloskirjautumisen jälkeen voimassa identiteetintarjoajan palvelussa, sisäänkirjautuminen takaisin MOOC-palveluun onnistuu ilman salasanaa. Siksi on tärkeää, että kirjauduttaessa ulos MOOC-palvelusta sessio päätetään myös identiteetintarjoajan palvelussa.
  ...
  \label{par:pääsyoikeuksien_myöntäminen_ohjelmakoodin_palauttamiseksi}
    ...
    Monipuolisempi vaihtoehto on MOOC-palvelun integroiminen esimerkiksi Githubiin\footnote{http://www.github.com}, joka tarjoaa git-versionhallinnan\footnote{http://git-scm.com} SaaS-palveluna. Käyttäjän koodisäilön (\emph{repository}) osoite kytketään MOOC-palvelun käyttäjätunnukseen, ja käyttäjä lähettää tehtävänsä versionhallinnan kautta Github-palveluun. Lähetyksen jälkeen MOOC-palvelu noutaa tehtävät Github-palvelusta.
  ...
  \paragraph{Helsingin yliopiston identiteettipalvelu (todo)} % (fold)
