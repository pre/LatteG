...
parts as they pass through the user-agent.
  ...
  \item[(E)]
  Facebook Platform luo pääsyoikeuspoletin (\verb!access_token!) ja lähettää sen MOOC-palvelun kohdassa (D) annettuun paluuosoitteeseen. MOOC käyttää pääsyoikeuspolettia, kun se kommunikoi loppukäyttäjän valtuuksin Facebook Platformin kanssa. Tämän vaiheen jälkeen MOOC tuntee käyttäjän perustiedot ja sähköpostiosoitteen.

  \end{description}

  Prosessin viimeisen vaiheen \verb!(E)! jälkeen käyttäjä on hyväksynyt MOOC-palvelulle pääsyoikeuden oman Facebook-profiilinsa perustietoihin. Käyttäjän suostumusvaihessa \verb!(C)! Facebook Platform hyväksyy pyynnön käyttäjän aiemmin myöntämää auktorisointia vastaan. Käyttäjältä ei kysytä auktorisoinnin hyväksymistä uudelleen, ennen kuin auktorisointi vanhenee tai käyttäjä poistaa auktorisoinnin. Lisäksi, jos käyttäjän oma Facebook-sessio on voimassa vaiheessa \verb!(B)!, MOOC-palveluun myöhemmin palaava käyttäjä voidaan autentikoida kysymättä salasanaa.

  Facebook Platformin myöntämä pääsyoikeuspoletti on voimassa vain rajallisen ajan ja se mitätöityy, jos käyttäjä vaihtaa salasanaansa tai kirjautuu ulos Facebookista. Pääsyoikeuspoletin uusiminen edellyttää OAuth-virtauksen suorittamista uudelleen. Jos MOOC tarvitsee käyttäjästä riippumattoman pääsyn resurssiin, sen on pyydettävä käyttäjän suostumusvaiheessa Facebook Platformilta \verb!scope!-parametrin välityksellä \verb!offline_access!-käyttöoikeutta~\cite{facebook_oauth2_doc}.

  OAuth-standardin mukaan auktorisointipalvelin tarjoaa virkistyspoletin (\verb!refresh_token!), kun asiakaalle myönnetään pääsy käyttäjän tietoihin silloinkin, kun käyttäjä ei ole läsnä~\cite{ietf_oauth2}. Asiakas voi virkistyspoletin avulla pyytää uuden pääsyoikeuspoletin ilman käyttäjän suostumusvaihetta: OAuthin yhtenä perusajatuksena on kannustaa pitkäikäisiin auktorisointeihin ja lyhytikäisiin pääsyoikeuspoletteihin. Facebook ei tätä kirjoitettassa kuitenkaan tarjoa koskaan virkistyspolettia, vaan myöntää sen sijasta rajoittamattomasti voimassaolevan pääsyoikeuspoletin~\cite{facebook_oauth2_doc}.
