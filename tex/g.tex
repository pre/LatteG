...
%    - Kerro blogien pitäjistä, miksi ovat merkittäviä lähteitä.
...
\label{sec:Kertakirjautumisstandardit}
  \begin{quote}
      ...
      \\--- Pudd'nhead Wilson~\cite{twain_eggs_1894}
  \end{quote}
...
\label{sec:mooc_palvelun_autentikointiratkaisun_arkkitehtuuri}
  ...
  \subsection{Autentikoituminen Google-identiteetillä: OpenID vai \\OAuth?} % (fold)
  ...
  Google on tarjonnut vuodesta 2008 lähtien autentikointirajapinnan OpenID 2.0 ja OAuth 1.0 -protokollien yhdistelmällä. Keväällä 2011 Google julkisti myös kokeellisen tuen\footnote{
  Tätä tutkielmaa kirjoitettaessa merkintä ''experimental'' katosi Googlen OAuth 2.0 -rajapintadokumentaatiosta tammikuussa 2012.
  } itsenäiselle OAuth 2.0 -rajapinnalle~\cite{google_oauth2_announcement_2011}. Facebook-identiteetin tapauksessa sovellettavan standardin valinta oli suoraviivaista, koska Facebook tarjoaa ainoastaan OAuth 2.0:n ainoana vaihtoehtona. Google-identiteetin osalta valintaa kahden protokollan väliltä täytyy perustella.

  Loppukäyttäjän kannalta ei ole merkitystä, mitä standardia MOOC-palvelu soveltaa yksittäisen identiteetintarjoajan suhteen. Loppukäyttäjä näkee MOOC-palvelun sisäänkirjautumissivulla Googlen sisäänkirjautumislinkin ja syöttää Googlen palveluun oman salasanansa. Sovellettavan tekniikan erot näkyvät sen sijaan ohjelmistokehittäjälle.

  Molemmissa protokollissa, OAuth 2.0:ssa ja OpenID 2.0:ssa, on samantyyppinen uudelleenohjauksiin perustuva arkkitehtuuri. OAuthissa käyttäjä myöntää valtakirjan minkä tahansa tyyppisiin resursseihin ja identiteetti on yhdenlainen resurssi muiden joukossa. OpenID 2.0 puolestaan jakaa ainoastaan identiteettiin liittyvät tiedot. Muuta yhteistä näillä kahdella protokollalla ei ole: kummallakin on omanlaisensa vaatimukset kolmannen osapuolen rekisteröimiselle ja kummallakin on erilainen tapa varmistaa pyyntöjen autenttisuus. Vuonna 2007 julkaistu OpenID 2.0 tukee auktorisointikerroksella vain OAuth 1.0 -laajennoksen käyttämistä.

  OpenID Connect on OpenID 2.0:n seuraaja, jossa OAuth 2.0:n auktorisointikerros on rakennettu suoraan sisään protokollaan. OpenID Connect on edeltäjäänsä yhteensopivampi ohjelmoitavien rajapintojen suhteen, mutta se ei ole taaksepäin yhteensopiva OpenID 2.0:n kanssa~\cite{sakimura_openid_connect_scopes_2011}. Koska OAuth 2.0 ei ole vielä lopullinen standardi, sen päälle rakentuva OpenID Connect on vielä sitäkin varhaisemmassa määrittelyvaiheessa\footnote{
  ...
  % - David Recordon 03/2010: http://daveman692.livejournal.com/349384.html
  %
  % "Since Facebook provides both identity as well as authorization, it’s critical that we get this piece right in order to have a complete solution."
  % - Luke Shepard
