...
%Eri järjestelmien pitää toimia keskenään yhteen, jotta sama identiteetti voidaan jakaa toisistaan riippumattomien palvelujen kesken. Suljetussa verkkoympäristössä tarpeet voivat olla heterogeenisia ja voidaan jopa käyttää samaa domainia. Julkisessa internet-ympäristössä hyvin erilaisia tarpeita ja palvelut todellakin eri domaineissa (vrt cookien jakaminen). Tarve yhteiskäyttöiselle standardille.
  ...
  OpenID Connect rakentuu suoraan OAuth 2.0 auktorisointikerroksen päälle, joten OpenID Connectin autentikointivaihe tapahtuu OAuth 2.0 -auktorisointipyynnön yhteydessä. Kun asiakas asettaa poletin hankkimisvaiheessa oikeuksien laajutta kuvaavan \verb!scope!-parametrin arvoksi \verb!openid!~\cite{sakimura_openid_c_nutshell_2012}, OpenID Connect -standardia tukeva auktorisointipalvelin käynnistää OpenID Connect -autentikointivaiheen.

  Auktorisointipalvelin palauttaa abstraktin OAuth-virtauksen (kuva~\ref{fig:oauth_abstract_flow}) vaiheessa \verb!(D)! pääsyoikeuspoletin lisäksi myös \emph{identiteettipoletin} (\verb!id_token!)~\cite{bradley_openid_c_id_token_2012}. Identiteettipoletti valtuuttaa asiakkaan ainoastaan käyttäjän identiteettiin liittyviin tietoihin, mikä mahdollistaa autentikoinnin ja auktorisoinnin löyhän kytkennän. OAuth 2.0 -standardi ei puolestaan määrittele autentikointia lainkaan, vaan pääsyoikeuspoletti toimii valtakirjana mihin tahansa resurssiin, johon asiakas on valtuutettu.

  OAuth 2.0 -auktorisointipalvelin ei missään vaiheessa kerro resurssin asiakkaalle käyttäjän identiteettiä. Resurssin asiakas \emph{luottaa} käyttäjän autentikoinnin onnistuneen, koska auktorisointipalvelin myönsi pääsyoikeuspoletin, jonka perusteella asiakas saa delegoidun pääsyoikeuden käyttäjäidentiteetin tietoihin. Siten asiakas saa tietoonsa käyttäjän identiteetin vasta sen jälkeen, kun se vastaanottaa identiteettiresurssin palvelimelta pääsyoikeuspoletin valtuuttamat käyttäjätiedot (kuva~\ref{fig:oauth_abstract_flow}, vaihe \verb!(F)!).

  Pääsyoikeuspoletti sisältää ainoastaan \emph{epäsuoran viitteen} siihen, että sen kantaja on resurssin omistaja~\cite{bradley_oauth_authentication_problem_2012}. Tämän vuoksi OAuth 2.0 ei ole autentikointistandardi, vaan OAuth ainoastaan simuloi autentikointiprosessia. OpenID Connect on puolestaan standardi nimenomaan autentikoinnin tarpeisiin.
...
Identiteettipolettiin kytketään tieto sen tarkoitetusta vastaanottajasta, joten sen soveltaminen loppukäyttäjän autentikointiin edellyttää asiakkaan ja autentikointipalvelimen jaetun salaisuuden (\verb!client_secret!) tuntemista.

  Identiteettipoletti mahdollistaa identiteettitietojen välittämisen suoraan osana identiteettipolettia. Kuten luvussa~\ref{sec:autentikoituminen_ja_web} esiteltiin, poletti voi olla viite identiteettitietoon tai se voi kryptografisin menetelmin sisältää identiteettitiedon itsessään. OpenID Connect -identiteettipoletti koodataan JSON Web Token (JWT) -muodossa käyttäen avaimena asiakkaan ja autentikointipalvelimen tuntemaa jaettua salaisuutta (\verb!client_secret!).  Identiteettipoletti sisältää seuraavat tiedot~\cite{sakimura_openid_c_nutshell_2012}:

% You could say the audience for the OAuth token is the protected resource and the audience for a authentication token is the RP.   They are not the same endpoint! \cite{bradley_oauth_authentication_problem_2012}.
  ...
  \begin{description}
    ...
    \item[user_id]: Identiteetintarjoajan kontekstissa pysyvästi yksilöllinen käyttäjätunniste. Identiteetintarjoaja ei saa koskaan antaa aiemmin myönnettyä tunnistetta toiselle käyttäjälle.
 ...
 %
 % The problem is that in the authentication case Websites do have a motivation to inappropriately reuse the access token.
 %
 ...
 %
 %   kommenteista: Eran believes MAC tokens are the solution. I however think there are probably better ways to solve it by being explicit about what endpoints a token is good for.

 OpenID:ssä samat virtaukset (auth code ja implicit) kuin OAuth 2.0:ssa. (onko myös client ja resource owner)

 Protected resources must be sure the access token is for them, that is sort of the point of them.
 Oauthissa accesscode mihin tahansa resurssiin ja aina käyttäjä on mukamas autentikoitu. Koska accesstoken ei rajattu sillä pääsee aina käsiksi käyttäjän perustietoihin.
 ...
 % subsubsection identiteettipoletti (end)
  ...
  \begin{quote}
    ''Great post by John Bradley on the huge security hole many IDPs have created by using plain OAuth, instead of OpenIDConnect, for authentication. We need to keep hammering away on this point both so IDPs realize the security problems they are creating for their partners, and to get RPs to realize how easily they can compromise their own security just because of the lack of a few additional lines of code. Years ago when Google first launched its proprietary equivalent of OAuth, called AuthSub, we had a big section at the bottom warning people not to use it for authentication for exactly this reason.''
