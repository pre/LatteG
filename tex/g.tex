...
\label{sec:mooc_palvelun_autentikointiratkaisun_arkkitehtuuri}
  ...
  }. OAuth 2.0:n määrittelytyössä vaikuttaa olleen alusta lähtien tavoitteena saada aikaiseksi autentikoinnista itsenäinen auktorisointikerros, ja vasta standardin valmisteluvaiheen stabiloitumisen jälkeen alkaa auktorisointikerroksen päälle rakentuvan OpenID Connect -autentikointikerroksen suunnittelu~\cite{recordon_oauth20_initial_2010, shepard_oauth_facebook_2010}.
  ...
  Eroina Googlen ja Facebook Platformin OAuth 2.0 -rajapintojen ovat ainoastaan päätepisteiden osoitteet ja yksittäisten parametrien arvot; OAuth-virtauksen vaiheet ovat täsmälleen samat kuin kuvassa~\ref{fig:oauth_auth_code_flow}. Koska OAuth 2.0 on auktorisointistandardi, sillä ei ole standardin määrittämää tapaa käyttäjän identiteettitietojen kysymiseksi. Tästä johtuen vaiheessa \verb!(B)! pyydettävien tietojen laajuutta kuvaavan \verb!scope!-parametrin arvo on Facebookilla \emph{email}, mutta Googlella \emph{\url!https://www.googleapis.com/auth/userinfo.email!}. Vastaavasti vaiheen \verb!(E)! jälkeen tapahtuva käyttäjän identiteettitietojen resurssi on kummallakin palveluntarjoajalla eri niminen.

  Vakioidun käyttäjätietojen päätepisteen määritteleminen on yksi OpenID Connect -standardin merkittävimmistä kontribuutioista. Käyttäjätietojen päätepiste on tavanomainen OAuth 2.0 -resurssi, mutta siinä missä OAuth 2.0 edellyttää päätepisteen tuntemista etukäteen, OpenID Connect mahdollistaa sen löytämisen dynaamisesti. Kuvassa~\ref{fig:oauth_auth_code_flow} esitetyssä OAuth 2.0 -prosessissa muodostetaan OpenID Connect -autentikointipyyntö siten, että asiakas määrittää \verb!scope!-parametrin arvoksi \verb!openid!~\cite{sakimura_openid_c_nutshell_2012}. Tällöin vaiheessa \verb!(E)! auktorisointipalvelin palauttaa pääsyoikeuspoletin lisäksi myös \emph{identiteettipoletin} (\verb!id_token!).

  Identiteettipoletin mahdollistaa autentikoinnin ja auktorisoinnin löyhän kytkennän. OAuth 2.0:n pääsyoikeuspoletti toimii valtakirjana mihin tahansa resurssiin, johon asiakas on valtuutettu. OpenID Connectin identiteettipoletti toimii valtakirjana ainoastaan identiteettitietojen resurssiin. Kun identiteettitietojen resurssin päätepiste on riippumaton muista resursseista, identiteettiresurssin palvelin voi olla itsenäisen muiden resurssien suhteen.
